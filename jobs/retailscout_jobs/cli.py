...
def cmd_build_features(feature_version: str) -> int:
    ...
    from .features.development_features import build_development_features
    ...
    try:
        with engine.begin() as conn:
            ...
            dev = build_development_features(conn, feature_version=feature_version)
    finally:
        engine.dispose()
    ...
    print(
        f"  development (config {dev.config_version}, "
        f"source release#{dev.source_release_id}, {dev.cells_written} cells)"
    )
    return 0


def cmd_build_pedestrian_baselines() -> int:
    from .db import get_engine
    from .features.pedestrian_baseline import build_sensor_baselines

    engine = get_engine()
    try:
        with engine.begin() as conn:
            result = build_sensor_baselines(conn)
    finally:
        engine.dispose()

    print(
        f"Built pedestrian baselines (baseline_version={result.baseline_version}, "
        f"window {result.window_start}..{result.window_end}): "
        f"{result.rows_written} sensor×day_type×daypart rows"
    )
    return 0
...
def cmd_build_scores() -> int:
    from .db import get_engine
    from .scoring.score import build_scores

    engine = get_engine()
    try:
        with engine.begin() as conn:
            result = build_scores(conn)
    finally:
        engine.dispose()

    print(
        f"Built scores for release#{result.release_id} "
        f"(score_version={result.score_version}, feature_version={result.feature_version}): "
        f"{result.rows_written} cell×profile rows"
    )
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    sub.add_parser(
        "build-scores",
        help="Compute per-cell profile suitability scores (analytics.location_score)",
    )
